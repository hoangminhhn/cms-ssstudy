...
const WordExamUpload: React.FC = () => {
  ...
  // New state: control whether PracticeConfig panel is visible (hidden by default)
  const [showPracticeConfig, setShowPracticeConfig] = React.useState<boolean>(false);
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-8 gap-4">
          ...
          {/* New switch to control visibility of PracticeConfig */}
          <div className="col-span-1 flex items-center gap-3">
            <Label className="mb-0">Hiển thị cấu hình luyện đề</Label>
            <div>
              <Switch checked={showPracticeConfig} onCheckedChange={(v) => setShowPracticeConfig(!!v)} />
            </div>
          </div>
          ...
          {/* Conditional: show PracticeConfig only when user enabled the showPracticeConfig switch AND group === 'Thi thử' */}
          {showPracticeConfig ? (
            group === 'Thi thử' ? (
              ...
              <div className="col-span-full mt-2 text-sm text-muted-foreground">
                Đã bật cấu hình luyện đề — vui lòng chọn Nhóm = "Thi thử" để hiển thị các tuỳ chọn.
              </div>
            )
          ) : null}
        </CardContent>
