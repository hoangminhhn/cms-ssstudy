...
const EditExamFormCategory: React.FC = () => {
  ...
  const handleAllowSubGroupsChange = (partId: string, checked: boolean) => {
    setParts(prev =>
      prev.map(part => {
        if (part.id === partId) {
          // Update parent allowSubGroups
          const updatedPart = { ...part, allowSubGroups: checked };
          // If splitIntoSubParts is enabled, sync all subParts allowSubGroups to parent value
  ...
  const handleAllowSubGroupsToggle = (partId: string, subPartId: string, checked: boolean) => {
    setParts(prev =>
      prev.map(part => {
        if (part.id === partId && part.subParts) {
          const updatedSubParts = part.subParts.map(sp => {
            if (sp.id === subPartId) {
              return { ...sp, allowSubGroups: checked };
            }
            return sp;
          });
          // Determine if all subParts have allowSubGroups true
          const allSubPartsChecked = updatedSubParts.every(sp => sp.allowSubGroups);
          return { ...part, subParts: updatedSubParts, allowSubGroups: allSubPartsChecked };
        }
        return part;
      })
    );
    ...
    setParts(prev =>
      prev.map(part => {
        if (part.id === partId) {
          ...
          // If enabling splitIntoSubParts and subParts exist, rename subParts to "Phần x.y"
          if (checked && updatedSubParts.length > 0) {
            ...
            updatedSubParts = updatedSubParts.map((sp, idx) => ({
              ...
              allowSubGroups: part.allowSubGroups ?? false, // sync allowSubGroups state
            }));
          }
    ...
    setParts(prev =>
      prev.map(part => {
        if (part.id === partId) {
          ...
          // After deletion, update allowSubGroups of parent based on remaining subParts
          const allSubPartsChecked = filteredSubParts.length > 0 && filteredSubParts.every(sp => sp.allowSubGroups);
          return { ...part, subParts: filteredSubParts, allowSubGroups: allSubPartsChecked };
        }
        return part;
      }),
    );
                                  ...
                                  <div key={subPart.id} className="relative border rounded-md p-4 bg-white dark:bg-gray-800">
                                    ...
                                    <label className="inline-flex items-center space-x-2 cursor-pointer select-none">
                                      <input
                                        ...
                                        onChange={(e) => handleAllowSubGroupsToggle(part.id, subPart.id, e.target.checked)}
                                        className="form-checkbox h-4 w-4 text-orange-600"
                                      />
