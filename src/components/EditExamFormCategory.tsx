...
const EditExamFormCategory: React.FC = () => {
  ...
  const handleChangeCategoryField = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (category) {
      setCategory({ ...category, examName: e.target.value });
    }
  };
  ...
  return (
    <div className="space-y-6 p-4">
      ...
      {/* Quản lý phần thi (tối giản chỉ thêm và xóa phần thi) */}
      <Card>
        <CardHeader>
                  ...
                  <div className="space-y-3">
                    {parts.map((part, index) => (
                      <div key={part.id} className="flex items-center gap-4">
                        ...
                        <div className="flex-1">{part.name ? part.name : `Phần ${index + 1}`}</div>
                        <Input
                          type="number"
                          min={0}
