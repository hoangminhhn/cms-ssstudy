...
const EditExamFormCategory: React.FC = () => {
  ...
  // Mock phần thi với trạng thái tương ứng
  ...
  const [filterStatus, setFilterStatus] = React.useState<'all' | PartStatus>('all');
  ...
  return (
    <div className="space-y-6 p-4">
      ...
      {/* Dải tag phân loại */}
      <div className="flex gap-4 px-4 mb-4 text-sm bg-gray-100 rounded-md p-2">
        <button
          onClick={() => setFilterStatus('all')}
          className={`cursor-pointer font-semibold ${filterStatus === 'all' ? 'text-black' : 'text-gray-600 hover:text-black'}`}
        >
          Tất cả ({counts.all})
        </button>
        <button
          onClick={() => setFilterStatus('published')}
          className={`cursor-pointer font-semibold ${filterStatus === 'published' ? 'text-black' : 'text-gray-600 hover:text-black'}`}
        >
          Đã xuất bản ({counts.published})
        </button>
        <button
          onClick={() => setFilterStatus('draft')}
          className={`cursor-pointer font-semibold ${filterStatus === 'draft' ? 'text-black' : 'text-gray-600 hover:text-black'}`}
        >
          Bản nháp ({counts.draft})
        </button>
        <button
          onClick={() => setFilterStatus('core')}
          className={`cursor-pointer font-semibold ${filterStatus === 'core' ? 'text-black' : 'text-gray-600 hover:text-black'}`}
        >
