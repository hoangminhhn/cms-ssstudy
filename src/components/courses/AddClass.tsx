...
const AddClass: React.FC = () => {
  ...
  // Inline edit for highlights
  const [editingHighlightId, setEditingHighlightId] = useState<string | null>(null);
  const [editingHighlightText, setEditingHighlightText] = useState<string>("");
  const editHighlightInputRef = useRef<HTMLInputElement | null>(null);
  ...
  useEffect(() => {
    if (editingHighlightId && editHighlightInputRef.current) {
  ...
  const handleDeleteHighlight = (id: string) => {
    ...
    // cancel edit if deleting edited one
  ...
  const saveEditHighlight = (id: string) => {
    const t = editingHighlightText.trim();
    if (!t) {
      toast.error("Nội dung không được để trống.");
      return;
    }
    setHighlights((prev) => prev.map((it) => (it.id === id ? { ...it, text: t } : it)));
    setEditingHighlightId(null);
    setEditingHighlightText("");
  ...
  return (
    <div className="space-y-6">
      {/* ... other cards above remain unchanged ... */}
      <Card>
        <CardHeader>
        ...
        <CardContent>
          <div className="space-y-3">
                ...
                highlights.map((h) => {
                  ...
                  return (
                    <div key={h.id} className="flex items-center gap-3 border rounded p-3 bg-white dark:bg-gray-800">
                      <button className="drag-handle p-1 text-gray-400 hover:text-gray-600" title="Kéo để thay đổi vị trí" aria-label="Kéo để thay đổi vị trí">☰</button>
            ...
            <p className="text-sm text-muted-foreground">Double-click hoặc nhấp vào mục để chỉnh sửa; dùng dấu tick để lưu.</p>
          </div>
        </CardContent>
      </Card>

      {/* keep CourseIncludes and other parts unchanged */}
