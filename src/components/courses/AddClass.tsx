...
import FeatureBlockEditor, { type FeatureBlock } from "@/components/feature/FeatureBlockEditor";
...
const AddClass: React.FC = () => {
  ...
  // Feature block state (from editor)
  const [featureBlock, setFeatureBlock] = useState<FeatureBlock | null>(null);
  ...
  useEffect(() => {
    ...
    if (!q) {
      ...
      setFilteredChapters(allChapters.filter((ch) => ch.title.toLowerCase().includes(q)));
    }
  ...
  // -- Short description and content editors (kept as before) --
  ...
  return (
    <div className="space-y-6">
      ...
      {/* other cards omitted for brevity in this file (kept as earlier) - chapters, price, fees, etc. */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="border rounded-md p-4 bg-white dark:bg-gray-800 min-h-[180px]">
              ...
              {selectedChapters.length === 0 ? (
                <div className="text-center text-muted-foreground py-8">Chưa có chương nào. Hãy thêm từ bên phải.</div>
              ) : (
      ...
      {/* Preview for webuser inside AddClass for convenience */}
      {featureBlock && (
        <Card>
          <CardHeader>
            <CardTitle>Preview hiển thị (Webuser)</CardTitle>
          </CardHeader>
          <CardContent>
            <FeatureListDisplay block={featureBlock} />
          </CardContent>
        </Card>
      )}
