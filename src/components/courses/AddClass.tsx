...
const AddClass: React.FC = () => {
  ...
  // -- Highlights (Thông tin nổi bật) feature: UPDATED to support double-click inline editing with check icon --
  ...
  // Sortable for highlights (preserve existing behavior)
  useEffect(() => {
  ...
  // New states for inline edit of highlights
  ...
  const saveEditHighlight = (id: string) => {
    const txt = editingHighlightText.trim();
    if (!txt) {
      toast.error("Nội dung không được để trống.");
      return;
    }
    setHighlights((prev) => prev.map((it) => (it.id === id ? { ...it, text: txt } : it)));
  ...
  return (
    <div className="space-y-6">
      {/* ... other cards remain unchanged above ... */}
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              ...
              <p className="text-sm text-muted-foreground">Kéo-thả để thay đổi thứ tự các mục; double-click để chỉnh sửa, nhấn dấu tích hoặc Enter để lưu.</p>
            </div>
          </CardContent>
        </Card>
